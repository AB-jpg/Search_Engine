...
def line_to_terms(line_string):
    #input the line in the file as a string and outputs a list of terms
    final_terms_list = []
...
def create_index(filenames, index, file_titles):
    ...
    #open content for each file
    #use the file name and title to update the file tititles dict
    #loop through the content, strip punctuation and parse out the terms, update index dict
    """
...
def search(index, query):
    ...
    for i, term in enumerate(query_list):
        ...
        if i == 0:
            ...
            result = common(result, temp_result)
    return result
